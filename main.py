...
def load_plugins(plugin_name):
    path = Path(f"FinalStrike/modules/{plugin_name}.py")
    spec = importlib.util.spec_from_file_location(f"FinalStrike.modules.{plugin_name}", path)
    load = importlib.util.module_from_spec(spec)
    load.logger = logging.getLogger(plugin_name)
    spec.loader.exec_module(load)
    sys.modules["FinalStrike.modules." + plugin_name] = load
    print("Final Strike Has uploaded " + plugin_name)


files = glob.glob("FinalStrike/modules/*.py")
for name in files:
    with open(name) as a:
        patt = Path(a.name)
...
print("\Final Strike 𝐃𝐞𝐩𝐥𝐨𝐲𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 ⚡\nMy Master ---> @STRIKE_OX")
